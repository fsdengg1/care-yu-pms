...
// .wrangler/tmp/pages-MPVv1W/functionsWorker-0.728988307775746.mjs
...
var DEFAULT_API_ORIGIN = "https://careyu-backend-api.aicareyuautomation.workers.dev";
function apiOriginFromEnv(env22) {
  const configured = typeof env22.CLOUDFLARE_API_ORIGIN === "string" && env22.CLOUDFLARE_API_ORIGIN || typeof env22.NEXT_PUBLIC_API_URL === "string" && env22.NEXT_PUBLIC_API_URL || DEFAULT_API_ORIGIN;
  return configured.replace(/\/$/, "");
}
__name(apiOriginFromEnv, "apiOriginFromEnv");
__name2(apiOriginFromEnv, "apiOriginFromEnv");
async function onRequest(context22) {
  const apiOrigin = apiOriginFromEnv(context22.env);
  const pathParam = context22.params.path;
  const segments = Array.isArray(pathParam) ? pathParam : pathParam ? [pathParam] : [];
  const incoming = new URL(context22.request.url);
  const target = `${apiOrigin}/api/${segments.join("/")}${incoming.search}`;
  const headers = new Headers(context22.request.headers);
  headers.delete("host");
  const method = context22.request.method;
  const init = {
    method,
    headers,
    redirect: "manual"
  ...
  if (method !== "GET" && method !== "HEAD") {
    init.body = context22.request.body;
  }
...
// .wrangler/tmp/bundle-f72JaJ/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-f72JaJ/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.728988307775746.js.map
